...
<div class="editViewContainer container-fluid">
        <div class="contents">
                <form id="EditAlgDragDropDoc" class="form-horizontal">
                        <div class="widget_header row-fluid">
                                <div class="span8"><h3>{vtranslate('LBL_ALGDRAGDROPDOC_SETTINGS', $QUALIFIED_MODULE)}</h3>&nbsp;{vtranslate('LBL_ALGDRAGDROPDOC_CONFIG_DESCRIPTION', $QUALIFIED_MODULE)}</div>
                        </div>
                        <hr>
    ...
    AppConnector.request(params).then(
            function(data) {                    
                    ...
                    if(data.result.enabled){
                        ...
                        Vtiger_Helper_Js.showMessage({text:app.vtranslate('JS_ALGDRAGDROPDOC_ACTIVATED'),type:'info'})
                    }else{
                        $('#'+data.result.tabid + ' .btnoff').addClass('active btn-primary');
                        $('#'+data.result.tabid + ' .btnon').removeClass('active btn-primary');
                        Vtiger_Helper_Js.showMessage({text:app.vtranslate('JS_ALGDRAGDROPDOC_DEACTIVATED'),type:'info'})
                    }
            },
            function(error,err){
